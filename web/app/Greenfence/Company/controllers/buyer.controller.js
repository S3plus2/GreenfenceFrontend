(function() {
	angular.module('company').controller('BuyerCtrl', ['$rootScope', '$scope', '$routeParams', '$compile', '$location', '$translate', '$timeout', 'DocumentDefinitionsService', 'BuyerService', 'CompanyDocumentService','ss_alert', 'ui', 
		...
		if($routeParams.buyerId){
			CompanyDocumentService.getCompanyDocuments({id: $routeParams.buyerId, type: "Buyer"}).$promise.then(function(data) {
			...
			BuyerService.getAssignedRequirements({id: $routeParams.buyerId, type: "Buyer"}).$promise.then(function(data) {
