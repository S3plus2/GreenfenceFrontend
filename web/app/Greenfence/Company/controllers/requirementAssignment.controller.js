(function() {
	angular.module('company').controller('RequirementAssignmentCtrl', ['$rootScope', '$scope', '$routeParams', '$compile', '$location', '$translate', '$timeout', 'DocumentDefinitionsService', 'SupplyChainService', 'RequirementAssignmentService', 'ProductService' ,'ss_alert', 'ui',
	  ...
	  DocumentDefinitionsService.getDocumentDefinitionByCompany().$promise.then(function(data) {
			...
			var showSelectedProducts = function() {
				...
				selected_products = getSelectedProductsElement()
			...
			$scope.assignRequirement = function() {
				...
				RequirementAssignmentService.save({requirements: selected_requirements,suppliers: selected_suppliers, products: selected_products }).$promise.then(success, error)
