...
define(['app'], function (app) {
    app.controller('SupplyChainCtrl', ['$rootScope', '$scope', '$compile', '$location', '$translate', '$timeout', 'ss_alert', 'ui', 'SupplyChainService',
        function ($rootScope, $scope, $compile, $location, $translate, $timeout, ss_alert, ui, SupplyChainService) {
            ...
            $scope.buyerProducts = function () {
                SupplyChainService.getProducts({
            ...
            $scope.inviteSupplier = function (company) {
                $scope.supply_chain_invitation = {
                    'product_csv': getSelectedProducts("#supplier_products"),
                    'invited_entity_id': company.invite_supplier.id,
                    'supply_chain_type': "supplier",
                    'invited_entity_type': company.invite_supplier.type
                }
            ...
            $scope.inviteBuyer = function (company) {
                $scope.supply_chain_invitation = {
                    'product_csv': getSelectedProducts("#buyer_products"),
                    'invited_entity_id': company.invite_buyer.id,
                    'supply_chain_type': "buyer",
                    'invited_entity_type': company.invite_buyer.type
                }
                SupplyChainService.save($scope.supply_chain_invitation).$promise.then(success, error)
            }
