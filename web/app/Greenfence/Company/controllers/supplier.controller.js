(function() {
	angular.module('company').controller('SupplierCtrl', ['$rootScope', '$scope', '$routeParams', '$compile', '$location', '$translate', '$timeout', 'DocumentDefinitionsService', 'SupplierService', 'CompanyDocumentService','ss_alert', 'ui', 
		function($rootScope, $scope, $routeParams, $compile, $location, $translate, $timeout, DocumentDefinitionsService, SupplierService, CompanyDocumentService, ss_alert, ui) {
			...
			if($routeParams.supplierId){
				SupplierService.getAssignedRequirements({id: $routeParams.supplierId, type: "Supplier"}).$promise.then(function(data) {
